...
namespace Eurekabank_Cliente_Consola_Unificado.Services
{
    ...
    public class RestDotNetService : IEurekabankService
    {
        ...
        public RestDotNetService(string baseUrl = "http://10.40.17.195:5111/api")
        {
            _baseUrl = baseUrl;
        ...
        public async Task<OperacionResult> RegistrarDeposito(string cuenta, double importe)
        {
            try
            {
                ...
                var response = await _httpClient.PostAsync($"{_baseUrl}/eureka/deposito", content);
                var responseContent = await response.Content.ReadAsStringAsync();
        ...
        public async Task<OperacionResult> RegistrarRetiro(string cuenta, double importe)
        {
            try
            {
                ...
                var response = await _httpClient.PostAsync($"{_baseUrl}/eureka/retiro", content);
    ...
    public class RestJavaService : IEurekabankService
    {
        ...
        public RestJavaService(string baseUrl = "http://10.40.17.195:8080/Eurobank_Restfull_Java/api")
        {
            _baseUrl = baseUrl;
            _httpClient = new HttpClient();
            _httpClient.Timeout = TimeSpan.FromSeconds(30);
        ...
        public async Task<OperacionResult> Login(string username, string password)
        {
            try
            {
                var usuario = new { usuario = username, clave = password };
                ...
                {
                    Exito = response.IsSuccessStatusCode,
                    Mensaje = response.IsSuccessStatusCode ? "Login exitoso" : "Credenciales inválidas"
                };
            }
            catch (Exception ex)
            {
